...
def main(args):
    ...
    for n in range(npts):
        ff = sim.get_farfield(nearfield, mp.Vector3(r*np.cos(np.pi*(n/npts)),r*np.sin(np.pi*(n/npts))))
        farfield_angles.append(np.angle(np.cos(np.pi*(n/npts)) + 1j*np.sin(np.pi*(n/npts))))
    ...
    with open(filename_dat, 'wb') as f:
        pickle.dump( {'params':params,
         'farfield_angles':np.array(farfield_angles),
         'farfield_power':np.array(farfield_power),
         'res_waveguide':res_waveguide} , f, pickle.HIGHEST_PROTOCOL)
