        ...
        for theta in thetas:
            for x in xs:
                for source in [0, 1]:
                    ...
                    commands.append(
                        "mpirun -np {6} python gc_outcoupler.py -period {0:} -FF {1:} -theta {2:} -x {3:} -source {4:} -filename {5:} > ./logs/{5:}.log".format(
...
sims = (
    np.shape(periods)[0] * np.shape(FFs)[0] * np.shape(thetas)[0] * np.shape(xs)[0] * 2
)
